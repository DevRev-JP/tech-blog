...
別の言い方では、**L1 はコパイロットだが補佐（検索・回答）のみ**、**L2 はコパイロット（副操縦士）として多少のアクションが取れる**、**L3 からはオートパイロット**です。オートパイロットのうち、**L3 は適宜人の確認を求める**、**L4 は特定領域で人の確認が不要**、**L5 は広い領域で人の確認が不要**、と区別できます。表の L1 で挙げる **RAG** は検索拡張生成、**GraphRAG** はグラフを用いた検索拡張の手法で、いずれも検索・回答の枠内に位置づけます。

![自動運転のレベル分けについて（国土交通省 別添3）](/images/mlit-autonomous-driving-levels.png)
_図：自動運転のレベル分けについて（国土交通省 別添3、官民ITS構想・ロードマップ等を基に作成）_
...
組織がこうした L4 のエージェントを導入・許容する際には、**精度**と**安全性**がよく議論されます。**精度**：現場で「正解」を出すには RAG だけでは不十分で、構造化データと非構造化データの密結合や異種システム間のデータを動的にリンクする能力が求められる、という見方があります。**安全性**：自律エージェントの暴走を防ぐため、複数システムにまたがる操作を一単位としたスナップショット・ロールバックや、権限・リソース枠に基づくガードレールが語られることが多いです。L4 では**1 人に対して 1 エージェント**（個人または特定ツール内）の範囲に留まるため、「誰が何を知る権利があるか」は顕在化しにくい一方、**1 エージェントが複数の人をサポートする**（1:N）に進むと、その議論が不可欠になります。本記事ではその観点は L5 で扱います。

---

## レベル 5：完全運転自動化に相当（個人・特定ツールに紐づかない）

自動運転の**レベル 5（完全運転自動化）**では、あらゆる条件下でシステムがすべての運転タスクと応答を担い、ドライバーは不要です。AI エージェントのレベル 5 は、本記事では**個人や特定ツールに紐づかない**領域として対応づけます。L4 は個人または特定ツールというスコープ内で完全自動であるのに対し、L5 はそのスコープに依存せず、組織の知能を個人やツールに縛られない形で扱う位置づけです。ここでは**1 エージェントが複数の人をサポートする**（1:N）形態になり、誰に何を見せるかを決める権限認識と、利用者ごとのパーソナライズが求められます。「誰が何を知る権利があるか」を扱う **Permission-ware（権限認識型）なナレッジグラフ**が鍵とされる、という議論があります。この領域に至る設計として、**企業向け（組織の）ナレッジグラフ**を土台に据える、という議論があります。L5 を MCP（Model Context Protocol）で実現できるのでは、と言われることがありますが、MCP の限界・課題については [MCP の課題とナレッジグラフ](https://zenn.dev/knowledge_graph/articles/mcp-knowledge-graph) を参照してください。

---

## 直近のトレンドをレベルで見る

ここでは、**記事の冒頭で触れた直近の動き**（OpenClaw の GitHub での注目、Microsoft Copilot Cowork への Claude 採用、国内の AI エージェント利用実態など）と、本文で例として挙げた製品・機能を、5 段階のレベルに載せて整理します。**この記事のレベル感**は、そうしたトレンドや製品を「どのレベルにいるか」で置く地図を提供するもので、個別の深掘りは各社の公式ドキュメントを参照してください。製品の実際の動作・レベルは利用形態に依存するため、導入時は公式情報の確認を推奨します。
...
- **AgentForce（Salesforce）** → **L3**（Salesforce プラットフォーム内で自律動作。Human in the Loop が設計原則のため。公式説明に基づく）
...
直近報じられたニュースのうち、本分類でレベルを判断する例を 2 つ挙げます。いずれも「どの運用か」でレベルが変わり得るため、公式発表や利用形態の確認が前提です。

1. **Microsoft Copilot Cowork × Claude（2026 年 3 月発表）**  
   Microsoft 365 上で Anthropic の Claude を利用し、ユーザーの一つの指示から資料作成・メール送信・スケジュール調整など長時間・多ステップのタスクを自律実行するエージェントです。クラウド上のサンドボックスで動作し、Work IQ で企業のメール・ファイル・会議などの文脈を参照します。本分類では、**人がタスクごとに指示・確認する運用なら L2**、**システムが監視し介入要求時のみ人が対応する運用なら L3** に位置づけられます。製品の実際のレベルは利用形態に依存するため、公式ドキュメントで確認してください。

2. **OpenClaw の 3 月リリース（v2026.3.x）**  
   GitHub で注目されている個人向け AI エージェントで、複数チャネル（WhatsApp, Slack, Discord 等）や音声・Canvas に対応します。2026 年 3 月に PDF 対応や Agent Context Engine プラグインなどのリリースがありました。個人のデバイス上で、人が介在せずにタスクを実行する形態として、本分類では **L4**（個人において完全自動）に位置づけられます。

---

## まとめ

AI エージェントの自律度を、**国が定義する自動運転のレベル（レベル 1〜5）** に沿って 5 段階で整理しました。**ドライバーによる監視**（L1〜L2）と**システムによる監視**（L3〜L5）の境界は L2 と L3 の間にあります。L1 は検索・回答のみで人が常時承認、L2 は人が常時監視・トリガー、L3 はシステムが監視するが介入要求に人が対応、L4 は個人または特定ツールにおいて人が介在せず完全自動、L5 は個人・特定ツールに紐づかない領域（企業向けナレッジグラフでこの領域に至る設計が議論される）です。

検索・回答に留まる L1 は多くの現場で有効で、RAG や Read 用途の MCP もこの範囲に含まれます。実装面では、L2 ではワークフロー・権限設計・スキルや MCP（Read/Write）など、L4 ではシステム間のイベント駆動と SoR（システムオブレコード）の操作や Text2SQL などがよく議論されます（詳細は [Connecting the Dots](https://zenn.dev/knowledge_graph/articles/ai-engineer-mindset-connecting-the-dots) を参照してください）。どのレベルを目指すかは組織や現場の文脈によります。国内では L1 から L2 への移行が進みつつ、組織利用では L3 から L4・L5 への進化が議論される動きがあります。
