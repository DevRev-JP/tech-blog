...
sequenceDiagram
    ...
    User->>Agent: 顧客Aの担当者と最新案件を教えて
    Agent->>kintone: 顧客情報を取得
    kintone-->>Agent: 顧客ID・社名
    Agent->>SmartHR: 担当者情報を取得
    SmartHR-->>Agent: 従業員ID・部署
    Agent->>Salesforce: 案件情報を取得
    Salesforce-->>Agent: 案件ID・ステータス
    Note over Agent: 3つの断片をコンテキストに積んで推論
    Agent-->>User: 回答（遅延あり、関係性が不正確）
```
...
**推論時データ収集型（Model-first）**: LLMが推論の過程でツールを呼び出し、必要なデータをその都度取得する設計です。記事内では以降「推論時データ収集型」と呼びます。「Model-first」はこの設計を「Memory-first」と対比するためのラベルであり、業界標準の用語ではありません。LangChainのAgentExecutor、OpenAI Function Calling、MCPの多くの実装がこのパターンです。

**事前統合型（Memory-first）**: 推論が始まる前に、関連データが統合済みの状態にしておく設計です。LLMはデータ収集ではなく、すでに整理された知識の上での推論に集中します。

2つのパターンを並べると、違いが明確になります。

**推論時データ収集型（Model-first）**
...
sequenceDiagram
    ...
    Agent->>Tool_C: データ取得
    Tool_C-->>Agent: 断片C
    Note over Agent: 断片A+B+Cをその場でつなぎ合わせる
    Agent-->>User: 回答
```
...
sequenceDiagram
    ...
    Note over KG: （事前）全システムのデータが統合済み

    User->>Agent: 質問
    Agent->>KG: 1回のクエリ
    KG-->>Agent: 関係性込みの構造化データ
    Agent-->>User: 回答（低遅延・一貫性あり）
```
...
DevRevはナレッジグラフを「Memory」として位置づけ、エージェントが推論に使う知識の基盤としています。カスタマーサポート・プロダクト開発・セールスにまたがるデータが、単一のグラフ構造に統合される設計です。
...
また、「SalesforceのAcme Corp」と「ZendeskのAcme」が同一企業かどうかを自動判定するEntity Resolution（名寄せ）が、同期インフラのレベルで組み込まれています。この設計の詳細は次回の記事で扱います。

公開資料でこの設計が強調するのは、「50番目のユースケースを5番目と同じコストで立ち上げられる」という点です。新しいシステムを統合するたびにエージェント側の実装を変える必要がなく、同期インフラにコネクタを追加するだけで済む構造になっています。これはシステム数が増えるほど、scatter-gather型との設計コスト差が広がることを意味します。
...
```mermaid
graph LR
    顧客 -->|情報管理| CRM[CRM / kintone]
    顧客 -->|問い合わせ| Support[サポートツール]
    顧客 -->|契約| Contract[契約管理]
    社員 -->|所属| HR[SmartHR]
    社員 -->|担当| 顧客
```
...
次の記事では、**ナレッジグラフの設計をどう始めるか**（スキーマ設計と名寄せ）を扱う予定です。

また別の記事で「**ツールが増えてもエージェントが賢くならない問題**」も扱います。ナレッジグラフの設計より先にエージェントアーキテクチャの問題に興味がある方は、そちらから読んでも独立して理解できます。
