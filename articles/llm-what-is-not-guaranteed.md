...
次トークン予測という計算系には、何が明示的に存在せず、それをシステムのどこで引き受けるべきなのか。
...
- スケールや学習でできることは増える。それでもシステムとして保証できることとは別である
- 保証が必要なものは、LLM の外側に正解を置く
- 文書取得、意味構造、状態管理、実行制約は、それぞれ別のシステム責務として扱う

外側の正解は、必ずしも一つではありません。会社の SoR は種類ごとに複数あるのが普通で、データレイクできれいに一つへまとまっている例は、規模が大きいほど少ないです。契約・在庫・権限・取引の状態は ERP 側に寄りやすい一方で、社内の会話、お客様からの問い合わせ、社内会議の議事録などは別の場所に置かれていることが多いです。種類ごとに「ここを正解とする」記録を決めて、そこを見に行けば十分です。

LLM に何を任せて、何を任せないかです。

## 1. LLM が見ている世界は文字でも単語でもない

LLM は通常、文字列をそのまま処理しているわけではありません。入力は Tokenizer によってトークンへ変換され、モデルはトークン ID から得た Embedding の系列を処理します。

トークン内部の文字情報が完全に「存在しない」わけではありません。トークンと元の文字列には対応があるので、学習によって綴りや文字の性質を内部表現として持つことはできます。

ただ、文字単位の構造は、最初から明示的な基本表現として与えられていません。ある単語が複数のサブワードに分かれていても、Transformer には「トークンの中を文字列として直接なめる」操作が最初から備わっているわけではありません。

そのため次のような処理は、自然言語の生成とは性質が違います。
...
モデルが学習によってうまくやることはあります。それでも「できること」と「その表現形式が操作を保証していること」は違います。本番で文字単位の正確さが必要なら、コード、Parser、Schema、制約付きの出力、Validator などを使う方がよいです。

これはモデルの能力不足というより、表現と仕事の粒度が一致していない、という設計の問題です。

うまくいくことがある、と仕組みとして保証されている、は違う。

## 2. Attention は内容による関係付けであり、状態機械ではない
...
- Causal Mask: 生成中のトークンより後ろ（未来）を見ないようにするマスク。Decoder-only では「未来を参照できない」という順序制約そのものにもなる
...
「Transformer には順序がない」と単純化するのは正確ではありません。順序は別途入れています。それでも、内容に基づく Attention そのものが、業務状態を更新する明示的な状態機械ではありません。
...
この遷移を、誰が正解として保持しているかです。会話履歴から状態を読み取れることと、状態を管理していることは違います。

長く動くエージェントでは、案件の状態・Workflow・権限・実行履歴を Context Window だけに置かない方がよいです。LLM は「承認へ進む」といった操作の候補を出せます。いまの状態の正解にはしません。

LLM は操作の候補を出せる。いまの状態の正解にはしない。

## 3. Residual Stream は、番地を指定して更新する状態ではない

Transformer の各層では、前の層の表現に変換結果を足し込む残差接続が使われます。深いネットワークへ情報を伝えるうえで、とても強い仕組みです。
...
Database や状態機械のように、「この番地の値だけを、こう書き換えた」と明示できる更新ではありません。あとから Attention や MLP が特徴を抑えたり変えたりはできます。「残差だから情報を消せない」という意味ではありません。

問題は、そこに業務システムとしての更新の約束がないことです。モデル内部には多くの特徴が重ねて表現されます。それは能力の源泉でもあります。一方で次のような契約を、残差の流れそのものに持たせることはできません。
...
だから長期の状態や業務上の正解は、モデル内部に置かない方がよいです。Ticket、Database、Workflow Engine、Knowledge Graph など、更新の意味を明示できる場所に置きます。

モデル内部の表現は強い。それでも「この値が正解である」という更新の約束は持てない。

## 4. Next-token Objective に真偽は含まれていない

LLM の事前学習で直接やっているのは、だいたい「次に出てきそうなトークンを当てる」ことです。命題が現実世界で真かどうかを示すラベルを、学習の目的そのものにしているわけではありません。

大量の学習で、文法・概念・関係・パターン・多くの事実を内部表現として持つことはできます。それでも次トークン予測の目的そのものには、「この命題は真である」「分からなければ棄権する」はありません。

Instruction Tuning や Preference Optimization、RLHF、Verifier、Process Supervision などで大きく改善はできます。それでも「トークンの出やすさ」と「命題への確信度」は同じではありません。
...
## 5. Sampling は業務制約ではない

LLM は確率分布を出します。推論ではその分布からトークンを選び続けて文章になります。Greedy、Temperature、Top-k、Top-p などで結果は変わります。Temperature を下げればばらつきは抑えられ、上げれば多様になります。
...
業務制約でもありません。たとえば次の条件は、Temperature を 0 にしても保証されません。
...
本番で優先したいのは、つまみの調整より「選べる範囲そのもの」を狭くすることです。Structured Output、JSON Schema、Constrained Decoding、Function Calling、型チェック、社内ルールのチェック、外での検証などがその例です。

正しいものを選びそうな状態を作るのではなく、間違った操作を採用できない系を作る。
...
だから推論の結果と、システムの状態は分ける。

## 7. LLM の外側は一枚岩ではない

何でも RAG に入れればよい、という話ではありません。外に出す対象ごとに責務が違います。
...
Knowledge Graph は単なる検索インデックスではありません。Agent が「この顧客に対して、この契約と社内ルールがどういう意味関係か」をたどるときの意味の層になります。形式レイヤは、意味構造といまの状態を使い、実行条件を決まった手順で評価します。
...
| 順序・状態 | Architecture / Context | 手順やいまの状態を推測に頼る | Workflow / 状態機械 |
...
これらの補償を足すことを「LLM が賢くなった」とは言いません。実際に起きているのは、確率的な言語モデルの周囲に、状態・意味・制約・検証・実行というシステムを組んでいる、ということです。

Database、Workflow、型、Policy Engine、Knowledge Graph、Test、監査ログ。生成 AI 以前からある地味な技術が、LLM の登場で不要になったわけではありません。むしろ LLM が非決定的だからこそ、重要性が上がっています。

## 9. 「ただの次トークン予測」という批判を否定する必要はない

「LLM はただの統計だ」は半分正しいです。統計的な目的から、内部に複雑な回路や、世界に対応する表現が生まれることはあります。「統計だから中身は空」とは言えません。「内部表現が生まれたから、世界を正しく理解している」とも言えません。

「Hallucination するから使えない」でもなければ、「RAG を付ければ Hallucination は解決する」でもありません。
...
「理解しているか」「思考しているか」を決着させなくても、システムは設計できます。本番で問うのは、どこまでをモデルに任せて、その出力をどの条件で採用するかです。
...
だから「返金してよいと思います」という感想だけで実行もできません。一方で、いつも「承認してください」で止まるエージェントも半人前です。人間が毎回承認するなら、その人間がボトルネックになります。社員がいつも「いいと思います」しか言わなければ評価は低いです。現場が欲しいのは次の動きです。

- 100% 返金してよいと確認できたなら、返金処理までやっておく
- 怪しいなら、何がどう怪しいかをまとめて承認を取りに来る

ここで分けるのは、LLM の感想と、`refund_allowed = true` が成立することです。後者には顧客の特定、契約、商品、社内ルール、権限、上限、いまの取引状態などが関係します。LLM は自然言語から意図を読めます。意味関係の正解、いまの状態、実行制約まで同じ確率モデルに預ける必要はありません。
...
条件を満たせば実行し、満たせないなら根拠付きで承認を取りに行く。ここまでできて、まともなエージェントになります。権限・追跡・元に戻す設計は [AI エージェントを本番に出せない本当の理由](https://zenn.dev/knowledge_graph/articles/kg-agent-production-safety) にまとまっています。

## 11. 次のトークンの先に置くもの

LLM は次のトークンを予測します。その単純な目的から、驚くほど複雑な能力が生まれます。「ただの次トークン予測だから使えない」は正しくありません。

内部に概念や世界に対応する表現が生まれることと、それを業務の正解として採用できることは別です。

本番では次のように責務を分けられます。

- 文書は RAG
- 意味構造は Knowledge Graph
- 状態は Database / Workflow
- 制約は形式レイヤ
- 実行は決まった手順のシステム
- 自然言語の入口と出口は LLM

LLM の外側に古いシステムを「戻す」のではありません。もともとシステムが持つべき責務を、LLM から分けているだけです。LLM がさらに賢くなっても、この原則は変わりません。

「SaaS is Dead」のような過激な言い方も見かけることがあります。シート単位のライセンスや、人が操作しやすい UI を売りにしてきた部分は、これから形を変えていくでしょう。一方で SoR は、LLM だけでは担えないレイヤです。契約・在庫・権限・取引の状態のように「ここを正解とする」記録の地位は、これからも必要です。

モデルが「できる」ことと、システムが「保証する」ことは違う。AI エージェントの設計で大切なのは、その境界を消すことではない。どこに境界を引くかです。
