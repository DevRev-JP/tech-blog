...
- **[「RAGなしで始めるナレッジグラフQA──コンテナで再現する比較検証」](https://zenn.dev/knowledge_graph/articles/kg-no-rag-starter)**
  ...
  - RAG: 5項目では 2～3/5、50項目では 0～1/5（スケール依存・ベクトル埋め込み由来の不安定性）
    - ※RAG の結果はベクトル空間における類似度計算や検索の不確定性により、実行ごとに変動する可能性があります
...
- **2025-10-24** — 実装レベルでの比較セクションを更新。RAG の結果が実行ごとに変動する可能性があることを明記し、より正確な測定結果の幅（5項目では 2～3/5、50項目では 0～1/5）を記載
