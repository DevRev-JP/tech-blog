...
「ナレッジグラフを使っています」という話を聞いてみると、実際には GraphRAG の話だったり、Neo4j を入れただけの話だったりすることが少なくありません。

また、「ナレッジグラフ ＝ グラフDB」だと思っている人も多いように感じます。グラフDB は保存・照会の技術であり、**何を Node に、何を Edge に置くか**は設計次第です。

LangGraph のように製品名に Graph が付くこともあります。結果として、中身の違うものが「ナレッジグラフ」や「グラフ」に丸められてしまいます。これが混乱の正体です。
...
社内で「グラフを使っている」と聞いたら、まず Node に何が入っているかを確認してください。人・製品・チケットならナレッジグラフ、処理ステップなら DAG、状態ならステートグラフ、という切り分けが最初の一歩になります。
...
「Neo4j を入れた」場合に限っても、RDF か Property Graph かを選ぶのは、上表の**ナレッジグラフ**の列を永続化するときの話です。社内の製品・顧客・チケットをエージェントが traversals しながら辿るなら Property Graph が向きやすく、オントロジーや標準語彙での推論、外部データとの URI 連携を重視するなら RDF が向きやすいです。比較の詳細は [RDF vs Property Graph](https://zenn.dev/knowledge_graph/articles/rdf-vs-property-graph-2025) にまとめています。
...
実際のフレームワークではステートグラフとワークフローグラフの境界が曖昧なこともあります。上で触れた LangGraph のように、1 つの製品が複数の役割を兼ねることもあります。ここで整理しているのは製品名ではなく、**グラフが表す役割**です。
...
続編「AI プラットフォームは 5 種類のグラフだけでは動かない」（執筆予定）では、プラットフォーム設計での特殊化、レイヤー間の連携、DevRev などの実装例を扱う予定です。本記事の分類だけでは足りない理由を、そこで具体的に見ていきます。
...
5 種類は AI エージェントに必要なグラフの**最低限の地図**です。本番のプラットフォームを組むには、この先にもう一段の解像度が要ります。
...
| 続編（実装・連携・執筆予定） | AI プラットフォームは 5 種類のグラフだけでは動かない |
