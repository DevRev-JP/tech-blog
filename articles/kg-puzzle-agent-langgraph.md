...
Part1 の LangGraph（グラフ取得 → 回答生成）は、この **BFF 先** の最小形です。Part0 では Skill / RAG が返しがちな **断片の束** と、グラフで固定した **関係付きコンテキスト** を同じ質問で比較します。
...
**Project Alpha** を題材に、Jira / Slack / Confluence 相当の **断片** と、移管後の **正式関係** をグラフで並べます。断片は意図的に「きれいな正答セット」と「現場っぽい混在セット」の2通りを用意しています。
...
推測で「Team B だろう」と決め打ちするのではなく、**差分を可視化して人間の判断を仰ぐ** 動きです。**Q3** では、グラフ上の現行 fact と Jira 古ドラフトの差を見せたうえで **確認の質問** に回します（Q2 の Skill 経路のように黙って確定しない例）。Part2 では同じ論点を **未解決矛盾** としてグラフに残し、チームの判断待ちに回せます。
