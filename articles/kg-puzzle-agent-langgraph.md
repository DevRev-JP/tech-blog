...
Part1 の LangGraph（グラフ取得 → 回答生成）は、この **BFF 先** の最小形です。Part0 では Skill 断片 **だけ** を渡した場合（上の従来に近い経路）と、グラフコンテキストを先に渡した場合を比較します。
...
| 矛盾時に推測せず確認・議論へ | Part0 概念 / Part2 ⚠ | 同上 |
...
experiment では Jira / Slack / Confluence は接続せず、**Project Alpha** の同一事実を断片（Skill 相当）とグラフの両方で表現します。断片は `tool_fragments.json` にあり、**意図的に「きれいな正答セット」と「現場っぽい混在セット」** の2通りを用意しています。
...
**Q2（矛盾断片）** では、上記の **現場混在** を再現した断片だけを渡します。Skill 側は Jira の古いドラフト（Team B 主担当）を採りやすく、グラフは `OWNED_BY` で **Team A** に固定されます。
...
Skill 断片だけの経路では、AI は **黙ってどちらかを選んで** 回答しがちです（Q2 では古い Jira 側に寄りやすい）。利用者から見ると、間違っていても **確信を持って答えたように見える** のが問題です。
...
推測で「Team B だろう」と決め打ちするのではなく、**差分を可視化して人間の判断を仰ぐ** 動きです。Part0 の compare デモは、移管済みの確定関係（`OWNED_BY`）で **即答する最小例** として実装しています。incoming エピソードとグラフを突き合わせて聞き返すフルフローは Part2 に近く、**未解決の矛盾をグラフに残す** 方で体感できます（後述）。
...
Part0 で述べた「どっちが正？」の確認は、Part2 では **未解決矛盾としてグラフに残す** 形で近づけています。営業の800万前提とエンジニアの3人月試算は、`eng_budget_conflict` として両方グラフ上に載り、search 出力の **⚠ 未解決** セクションに出ます。AI が Slack で聞き返す UI までは experiment では再現しませんが、**矛盾を推測で潰さず、チームの議論の入力にする** 地図として機能します（[1人1AIから…](#1人1aiからチーム会議の中のaiへ) の「人間が議論し、AI が手足になる」と対になります）。
