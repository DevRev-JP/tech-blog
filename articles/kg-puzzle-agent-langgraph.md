...
:::message
**前提知識**: 本記事は独立して読めます。Skill 層・コンテキストグラフ・GraphRAG との関係は、文末の **関連記事** にまとめています。
:::
...
| Graph Traversal Contract | グラフの読み方 | 別記事で扱う |
...
後述の Part0〜2 で、この **BFF 先** の流れを段階的に確認します。Part0 では Skill / RAG が返しがちな **断片の束** とグラフの **関係付きコンテキスト** を比較し、Part1 では LangGraph（グラフ取得 → 回答生成）で地図を先に渡す最小形を示します。
...
**Project Alpha** を題材に、Jira / Slack / Confluence 相当の **断片** と、移管後の **正式関係** をグラフで並べます。同じ題材で Skill 断片とグラフを比較するため、Part0 では **3つの質問パターン**（以下 **Q1 / Q2 / Q3**）を使い分けます。
...
グラフ側は、移管完了後の **正式な関係** だけを `Project Alpha -[:OWNED_BY]-> Team A` として固定しています。
...
推測で「Team B だろう」と決め打ちするのではなく、**差分を可視化して人間の判断を仰ぐ** 動きです。**Q3（矛盾→確認）** では、グラフ上の現行 fact と Jira 古ドラフトの差を見せたうえで **確認の質問** に回します（Q2 の Skill 経路のように黙って確定しない例）。Part2 では同じ論点を **未解決矛盾** としてグラフに残し、チームの判断待ちに回せます。

---

## Part1：完成図を先に渡す LangGraph エージェント

Part1 では、質問者ごとに見えるグラフ範囲が変わる **権限** を、experiment 上の `user_guest`（ゲスト）と `user_tanaka`（社内ユーザー）で示します。
...
グラフコンテキストを **先に** 取得し、その後に LLM が回答します。権限はパストラバーサル時点で効き、到達不能なノードはコンテキストに含めません。同じ質問でも **Skill 断片 + 禁止指示** では秘匿情報が漏れうる一方、**グラフ + ゲストユーザー** では取得段階で遮断される、という対比がポイントです。

---

## Part2：チームの記憶（なぜ800万かを説明する）

**Project Alpha 拡張（顧客X）** の2026年6月第4週ストーリー（500万 → 800万 → 10月リリース予定 → エンジニア試算の矛盾）を、時系列グラフライブラリ **Graphiti** で ingest します。Skill に最新値だけ書いても **なぜ500万が無効か** は監査しにくい、という論点がここでの焦点です。

**SSOT**（Single Source of Truth：どのファクトが正本かの運用ルール）で失効・置換を決め、**as-of クエリ**（「ある時点では何が正だったか」を問う検索）と **視点フィルタ**（営業 / エンジニアなど）で答えを切り替えます。ingest → SSOT 適用 → as-of（+ 視点）の3段構成です。**monday** as-of では500万のみ、**today** as-of では800万・10月予定・未解決矛盾が共存します（いずれも experiment 上の日付ラベル）。`history` では 500→800 の変遷と置換理由を表示し、Part1 の「参照したグラフ」と対で **根拠チェーン** という語彙で揃えています。

Part0 で述べた「どっちが正？」の確認は、Part2 では **未解決矛盾としてグラフに残す** 形で近づけています。営業の800万前提とエンジニアの3人月試算は、両方グラフ上に載り、search 出力の **⚠ 未解決** として可視化されます。推測で片方を消さず、**議論用の確認例** まで出してチームの判断待ちに回せます。
...
- 2026-06-28: 初版公開。Part0 の Q1/Q2/Q3 初出順整理、読後インサイト・Part2 用語・表記を補強
