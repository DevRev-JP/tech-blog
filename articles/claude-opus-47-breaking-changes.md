...
### 2) 外部レイヤ（ナレッジグラフ/コンテキストグラフ）で LLM を制御する
...
- [Claude の外側にコンテキストグラフを置くと、速く・正確に・トークンを抑えられる](https://zenn.dev/knowledge_graph/articles/context-graph-improves-llm)
...
その例として、DevRev は Claude に対して Shared Memory（同社の文脈ではコンテキストグラフと表現）を提供する形を取ります。
...
- Shared Memory（コンテキストグラフ）型: 権威ある関係（typed edge）を “読む” 前提で、join/filter を決定的に処理し、必要な結果セットだけを Claude に渡す

投稿内では、同一ビジネスクエリの反復という前提で、**トークン量を約 95% 減（約 3.2M → 約 157k）**、時間も **約 5.5 倍高速化**といった目安が示されています（環境・データ量・実装に依存するため、自社では実測が前提です）。

中規模以上のユースケースで「構造化＋非構造化」を統合し、LLM の更新（Opus 4.7 のような破壊的変更や挙動変化）の影響を受けすぎない運用を目指す場合、この手の Shared Memory / コンテキストグラフという外部レイヤは、選択肢の一つになります。
...
- 2026-04-18: 初版公開
- 2026-05-21: Context Graph → コンテキストグラフに統一、記事リンクタイトルを更新
