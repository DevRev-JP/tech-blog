...
```mermaid
graph TD
    U[ユーザー: 田中] -->|所属| T[チーム: サポート]
    T -->|ロール| R[ロール: サポート担当]
    R -->|アクセス可| D1[リソース: 顧客チケット]
    R -->|アクセス可| D2[リソース: 顧客プロフィール]
    R -->|アクセス不可| D3[リソース: 財務データ]
...
エージェントが意思決定を下した際、その時点でKGから参照したコンテキスト（どのノードを見たか、どのエッジを辿ったか）を記録します。これにより「何をしたか」だけでなく「なぜそう判断したか」が後から追跡できます。これが3つの問いのうち「追跡可能性の問い」への設計回答です。
