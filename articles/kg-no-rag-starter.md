...
- **embedding モデル進化時** — RAG は全データ再処理・再チューニング必須。KG は無影響
- **LLM 変更時** — RAG はプロンプト再調整必須。KG は無影響
- **スケーリング** — RAG のベクトル検索は計算コスト増加。KG は線形時間で高速検索

**規模の大きさが重要**：Wikidata（8,800万エンティティ）クラスでは、RAG のベクトル検索は実用的でなく、KG の Cypher クエリが高速実行可能。

**実務的な結論**：初期段階では RAG が低コストで有効。しかし、**長期的・大規模運用を視野に入れると、KG への投資が ROI が高い**。理想は「クエリが KG で答えられるなら KG、探索が必要なら RAG を補助」という役割分担です。
