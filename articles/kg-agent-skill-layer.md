...
ナレッジグラフとは、エンティティ（人・組織・製品・チケットなど）とそれらの関係性を構造化したグラフ型のデータ基盤です。エージェントが「何を知っているか」を事前に整理して置いておく場所です（詳細は[AIエージェントが毎回データを取りに行く設計の限界](https://zenn.dev/knowledge_graph/articles/kg-agent-memory-first-design)を参照）。
...
| **[ツールを100個並べてもAIエージェントは賢くならない](https://zenn.dev/knowledge_graph/articles/kg-agent-skill-layer)** | **Skill層による設計（本記事）** |
