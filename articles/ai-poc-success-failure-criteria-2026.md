...
AIプロジェクトの失敗の多くは、解決策（Solution）ありきで始まることに起因します。「RAGを使って検索システムを作りたい」「GPT-5.xの最新バージョンで自動応答させたい」といった要望は、依頼者が訴える「症状」や（素人判断の）「処方箋」に過ぎません。LLMは進化が早くモデルと疎結合にしないと陳腐化しやすく、業務プロセスの進化はそれより遅いことが多いです。最新モデルで効く業務が、売上・利益に直結するとは限らない点にも注意します。
...
- **既存の解決策を再構築する** — 狭い要件（「この問い合わせを自動返信したい」）に対し、「もしAIが過去の類似チケットやドキュメント、開発ログまですべて理解した上で自律的に回答し、グレーゾーンだけは人間が承認するようにしたらどうか」と、**より大きな文脈（Big Picture）**を提示する。
- **「ツール」ではなく「成果」で合意する** — 導入するのはAIツールだが、合意すべきゴールは「問い合わせ対応時間の50%削減」「エスカレーション率低下」といった**ビジネス成果**にする。

**補足：9ブロック・モデル（言われたことをそのまま処方しないための質問フレームワーク）**

「この機能が欲しい」という依頼をそのまま受けず、診断とビジョン拡大を体系的に進めるための**質問のマトリックス**が、9ブロック・モデルです。**3つの会話ステップ（列）× 3つの質問タイプ（行）**で構成されます。下の表がその9ブロックです。各マスで、該当するタイプの質問を投げかけます。

|                             | **C1. 原因を診断する**<br>Diagnose Reasons           | **C2. 影響を探る**<br>Explore Impact                                         | **C3. あるべき姿を描く**<br>Visualize Capabilities                       |
| --------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **Open**<br>（広く聞く）    | なぜ今、それを検討しているのですか？                 | ご自身以外に、誰がどのような影響を受けていますか？                           | もし魔法の杖があったら、理想的にはどうなって欲しいですか？               |
| **Control**<br>（誘導する） | それは主にXが原因ですか、それともYも要因ですか？     | それは単なる遅延ですか、それとも他部門・SLAにも影響していますか？            | もしXだけでなく、YやZも自動化できるとしたらどうですか？                  |
| **Confirm**<br>（合意する） | つまり、主な原因はXとYだという認識で合っていますか？ | サポートだけの問題ではなく、製品開発やCSATにも影響しているということですね？ | つまり、AができればBという成果が得られる。これが求めている解決策ですね？ |

会話は **C1 → C2 → C3** の順で進め、各ステップでは **Open → Control → Confirm** の順で質問を重ねて認識を固めます（表の列が会話の3ステップ、行が質問の3タイプです）。

例：AIエージェント導入の検討では、(1) 診断で「なぜ今検討し始めたか」をOpenで聞き、Controlで「繰り返し作業か、トリアージの問題か」と絞り、Confirmで「単純作業と手動トリアージが主因ですね」と合意する。(2) 影響で「サポート以外に誰が困っているか」を聞き、「エンジニアの集中やSLAにも影響している」と広げる。(3) 能力で「応答を速くするだけでなく、感情分析や自動トリアージまでできたらどうか」を提示し、「KB・チケット・Jiraを統合してハンドオフまでできれば、SLAとCSATの目標達成が理想の姿ですね」とConfirmする。このように、単なる「ツール導入」の話を**より大きな文脈（Big Picture）**へ書き換えることができます。
