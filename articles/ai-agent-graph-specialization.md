...
第1部でも触れたとおり、Obsidian や LLM Wiki のような Markdown ウィキはナラティブ層として有効です。一方、権限・同一性・監査・依存のような問いごとの Edge は、リンク付き MD だけでは型として持てません。特殊化は、その不足を用途別に名前付けしたものです。
...
6 つ全部を一度に設計する必要はありません。先に詰まった問いから 1 つ足す、という順序が現実的です。試作（PoC）では LangGraph だけで十分だったのに、本番投入で権限だけ先に破綻する、というパターンがよくあります。
