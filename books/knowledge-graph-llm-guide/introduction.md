...
**実務メモ：** ローカル環境のセットアップは `podman-compose`（またはDocker Compose）で行います。Neo4j + Ollamaの全スタックを1コマンドで起動する `podman-compose.yml` を第6章で提供しています。M1/M2 MacおよびLinux（x86_64/ARM64）に対応しています。
