...
**ポイント**: LLM は制約候補を JSON 化させ、最適化は OR-Tools ソルバに任せます。
...
- **SQL Layer vs SQL Unsafe**: パラメータ化クエリ vs 文字列連結（SQL インジェクションのリスク）
- **Policy Layer vs Policy Unsafe**: OPA による外部化 vs if/else ベタ書き（変更の困難さ）
