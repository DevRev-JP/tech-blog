...
現実のシステムでは、RAG パイプラインの後ろに LLM（GPT など）がついて、検索結果から自然な回答を生成します。しかし、その LLM でも **テキスト検索の曖昧性は超えられません**。詳しくは、姉妹記事「[RAGを超える知識統合](https://zenn.dev/knowledge_graph/articles/beyond-rag-knowledge-graph)」で 5 つの意味問合せ型を紹介しています。
