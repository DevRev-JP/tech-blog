...
本 PoC で **実装している範囲** と **本番で別途必要な要素** を整理します。設計思想（地図を LLM の前に置く・権限は取得段階・時系列は SSOT・矛盾は聞き返す）は本番適用に有効ですが、**コードの多くは検証用の最小構成** です。

| 見え方・用語 | PoC の実装 | 本番で置き換えるもの | 本番適用時の留意点 |
|--------------|------------|----------------------|-------------------|
| **Skill / MCP**（Part0 A） | `tool_fragments.json` を **プロンプトに貼るだけ**。ツール選定・MCP プロトコル・並列取得なし | Jira/Slack/Confluence MCP、RAG、エージェントのツールルーティング | MCP プロトコルは未使用（断片は JSON 直渡し） |
| **グラフ検索**（Part0 B / Part1） | 質問からエンティティは **Project Alpha 固定**（`graph_retriever.py`） | エンティティリンク、GraphRAG、LLM→Cypher、全文+グラフ | 権限付き Cypher のパターンは本番相当。エンティティ解決は PoC 簡略 |
| **LangGraph**（Part1） | `retrieve_context → generate` の **2 ノードのみ**。分岐・HITL なし | 矛盾時の `clarify` 分岐、Human-in-the-loop、チェックポイント | 地図→回答の順序が検証の焦点。分岐・HITL は本番で追加 |
| **Q3 矛盾検出** | `OWNED_BY` と断片を **正規表現** で突合（`conflict_clarify.py`） | ingest パイプライン、スキーマ制約、承認ワークフロー、Graphiti invalidation | 聞き返しフローは本番相当。検出は PoC 向け簡略実装 |
| **Q3 聞き返し文** | 決定論テンプレ + Ollama **1 プロンプト**（LangGraph 非経由） | Slack スレッド返信、LangGraph 分岐、チケット起票 | チャット UI・ワークフロー連携は本番で実装 |
| **Part2 Graphiti ingest** | Graphiti + Ollama。抽出結果は **モデル依存** | 本番 LLM、監査ログ、再 ingest ポリシー | ingest パイプラインは本番相当。抽出品質はモデル依存 |
| **Part2 SSOT** | `temporal_rules.yaml` を ingest 後に Neo4j へ書き戻し。不足時は `DEMO_SSOT` で **MERGE 補完** | Git 管理の運用ルール、DataOps、承認済み canonical fact | SSOT 適用の思想は本番相当。`DEMO_SSOT` は PoC 向けデータ補完 |
| **Part2 議論用行** | 固定文言（`demo_temporal.py` の `[デモ固定]` テンプレ） | Q3 と同型の LLM 聞き返し or 人間ファシリテーション | 未解決矛盾の可視化が目的。Slack 等 UI は PoC 範囲外 |
| **マルチプレイヤー / Claude Tag** | 記事の **設計議論のみ**。CLI 単一セッション | Slack 連携、共有 @Claude、会話追跡 | Slack 連携・共有操作は PoC 範囲外 |

**本番への写像（Part ごと）**

| Part | PoC で確認できること | 本番で追加が必要な要素 |
|------|----------------------------|----------------------|
...
**目的**: 同一事実を断片（Skill **相当** — MCP 非接続）とグラフの両方で表現し、推測依存の差を体感します。A モードは **JSON をプロンプトに貼っているだけ** です（[PoC スコープと本番環境の差](#demo-vs-production)）。
...
**目的**: グラフコンテキストを LLM の **前** に渡す LangGraph エージェントと、権限をパストラバーサルで効かせる設計を体験します。**2 ノードのみ**（本番の分岐・HITL は別途 — [PoC スコープと本番環境の差](#demo-vs-production)）。記事後半の **マルチプレイヤー向け地図** では、同じ共有 @Claude でも **人ごとに見える範囲が違う** 前提の最小形です（[地図の基盤](#map-foundation)）。
...
**目的**: 2026年6月第4週の予算・工期更新（500万 → 800万 → 10月予定 → エンジニア矛盾）を Graphiti + SSOT + as-of クエリで扱い、**なぜ800万か** を根拠チェーンで説明します。ingest は本番相当の構成ですが、`DEMO_SSOT` による MERGE 補完と `[デモ固定]` 議論用行は PoC 向けの簡略化です（[PoC スコープと本番環境の差](#demo-vs-production)）。`sales` / `eng` 視点と未解決矛盾は、記事後半の **チーム会議内 AI** に必要な地図素材です（Slack 共有操作そのものは再現しません。[地図の基盤](#map-foundation)）。
