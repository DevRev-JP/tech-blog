...
```
[1] Graphiti ingest  … 週次エピソード（YAML）からファクト抽出（LLM）
[2] SSOT 適用       … temporal_rules で valid_at / invalid_at を確定（LLM 非依存）
[3] as-of 検索      … 「ある日付・ある視点」で有効ファクトだけ返す（Neo4j 直接）
```

**ポイント**: Part2 は ingest 品質にモデル依存しますが、**失効・正本は SSOT が決定論的に上書き** するため、デモの骨格は安定します。
