...
demo_done() {
  ...
  echo ""
  echo "  Neo4j Browser: http://localhost:7474  （Cypher: README 付録 Neo4j）"
...
1. グラフ投入
   ./run_demo.sh seed
   → Neo4j Browser: README 付録 Neo4j（Part0/1 用 Cypher · Part2 未実行時）
...
7. Neo4j Browser で目視 — README 付録 Neo4j（Part2 用 Cypher）
...
cmd_part2() {
  ...
  unset DEMO_BATCH
  echo ""
  echo "→ Part2 完了 · Neo4j Browser: http://localhost:7474  （README 付録 Neo4j · Part2 用 Cypher）"
}
